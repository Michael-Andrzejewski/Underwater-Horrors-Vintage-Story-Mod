...
public class EntityBehaviorDeepSerpentAI : EntityBehaviorOceanCreature
{
    ...
    // ── Flee after damage ─────────────────────────────────────────────
    // While > 0, proximity aggro is suppressed so a fresh flee widens
    // the distance instead of instantly re-aggroing at close range.
    private float fleeAggroSuppressTimer;

    // ── Enrage after provoke ──────────────────────────────────────────
    // While > 0, the serpent is committed: flee-from-damage rolls are
    // skipped and a finished strike re-enters the attack charge instead
    // of spiraling back out.
    ...
    private void SetNextSpiralStep()
    {
        ...
        radiusTransitionDuration = config.AggroTime(
            config.DeepSerpentSpiralStepDurationMin +
            (float)(rand.NextDouble() *
    ...
    private void OnStalking(float deltaTime)
    {
        ...
        float effectiveOrbitSpeed = config.SerpentOrbitSpeed * config.DeepSerpentOrbitRadius / radius * (float)SpeedScale;
    ...
    private void OnAttacking(float deltaTime)
    {
        ...
        else if (isStriking)
        {
            ...
            if (attackAnimTimer >= StrikeDuration)
            {
                ...
                if (enrageTimer > 0)
                {
                    // Enraged: skip the spiral-out and line up the next
                    // strike as soon as the cooldown allows.
                    TransitionTo(SerpentState.Attacking);
                }
                else
                {
    ...
    public override void OnEntityReceiveDamage(DamageSource damageSource, ref float damage)
    {
        ...
        if (enrageTimer <= 0)
        {
            double chance = damage * config.AggroInverseChance(config.SerpentFleeChancePerDamage);
            ...
            entity.World.Rand.NextDouble() < config.AggroChance(config.SerpentProvokeChance))
        {
            if (config.DebugLogging)
                UnderwaterHorrorsModSystem.DebugLog(entity.Api,
                    $"DeepSerpent provoked by {damage:F1} damage, " +
